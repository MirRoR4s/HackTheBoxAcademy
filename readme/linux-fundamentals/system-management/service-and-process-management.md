...
```bash
MirRoR4s@htb[/htb]$ systemctl start ssh
```
...
```bash
MirRoR4s@htb[/htb]$ systemctl enable ssh
...
```bash
MirRoR4s@htb[/htb]$ ps -aux | grep ssh
...
```bash
MirRoR4s@htb[/htb]$ systemctl list-units --type=service
...
```bash
MirRoR4s@htb[/htb]$ journalctl -u ssh.service --no-pager
...
```bash
MirRoR4s@htb[/htb]$ ping -c 10 www.hackthebox.eu

MirRoR4s@htb[/htb]$ vim tmpfile
[Ctrl + Z]
...
```bash
MirRoR4s@htb[/htb]$ echo '1'; echo '2'; echo '3'
...
```bash
MirRoR4s@htb[/htb]$ echo '1'; ls MISSING_FILE; echo '3'
