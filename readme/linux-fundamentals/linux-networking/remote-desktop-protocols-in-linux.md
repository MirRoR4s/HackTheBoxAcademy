...
Windows、Linux、macOS都使用了远程桌面协议以提供远程的、图形化的访问系统的能力。系统管理员在许多场景中都能够用到远程桌面协议，比如故障排除、软件及系统更新、远程管理系统等。因为管理员需要远程连接到被管理的系统，所以他们需要使用适当的协议。此外，若他们想要远程地在系统上安装应用程序，那么他们需要使用不同的协议连接到系统。最常用的两种远程桌面协议是用于Windows的RDP以及用于Linux的VNC。
